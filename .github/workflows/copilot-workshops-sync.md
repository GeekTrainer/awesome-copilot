...
4. If a local mirror **already exists** and **no commits** were found since the last sync, stop here and call the `noop` safe output with a message like: "No new commits found in `github-samples/copilot-workshops@main` since last sync (`<last_synced_sha>`). No action needed." Then update the cache with the latest SHA.
...
Before opening the PR, write an updated `copilot-workshops-sync-state.json` to `cache-memory` with:
